...
within the `parameters` field of the `SqlInput` input object. See the other GraphQL schema
...
The Subgraph:SQL Service can be enabled or disabled by setting the `GRAPH_GRAPHQL_ENABLE_SQL_SERVICE`
environment variable to `true`. This allows clients to execute SQL queries using the
...
To enable the Subgraph:SQL Service:
...
permitted to be used within SQL queries executed by the Subgraph:SQL Service:
